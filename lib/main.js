...
module.exports = {
  ...
  activate(_state, { signal } = {}) {
    ...
    this.manager.setCapabilityInitializer(() => this.ensureProviders());
    this.manager.setAdapterInitializer(() => this.ensureCustomServers());
  ...
  provideSymbol() {
    return this.ensureProviderFacades().symbol;
  },
  provideHover() {
    return this.ensureProviderFacades().hover;
  },
  ...
  async format() {
    const editor = lumine.workspace.getActiveTextEditor();
    if (!editor) return;
    const edits = await this.ensureProviders().codeFormatProvider.formatFile(editor);
