...
module.exports = {
  activate() {
    ...
    this.manager.setFileOperationsExecutor(this.fileOperationsExecutor || null);
      ...
      lumine.commands.add("lumine-workspace", {
        ...
        "ide-client:fold-server-ranges": {
          ...
          didDispatch: () => {
            ...
            if (editor) return this.documentFeatures.foldRanges(editor);
          },
        },
        "ide-client:expand-selection-range": {
          ...
          didDispatch: () => {
            const editor = lumine.workspace.getActiveTextEditor();
            if (editor) return this.documentFeatures.expandSelectionRanges(editor);
          },
        },
        "ide-client:select-linked-ranges": {
          ...
          didDispatch: () => {
            const editor = lumine.workspace.getActiveTextEditor();
            if (editor) return this.documentFeatures.selectLinkedRanges(editor);
          },
        },
        "ide-client:color-presentation": {
          ...
          didDispatch: () => {
            const editor = lumine.workspace.getActiveTextEditor();
            if (editor) return this.documentFeatures.colorPresentations(editor);
          },
        },
      }),
    );
  ...
  provideHyperclick() {
    return this.documentFeatures.hyperclickProvider;
  },
  get sessionMenu() {
  ...
  consumeLinterRegistry(registerIndie) {
    ...
    const publish = ({ session, uri, diagnostics }) => {
      ...
      const editor = lumine.workspace
        .getTextEditors()
        .find((candidate) => candidate.getPath() === batch.filePath);
      const enabled = editor
        ? this.manager.featureEnabled(adapter, "diagnostics", editor)
        : this.manager.featureEnabledForPath(adapter, "diagnostics", batch.filePath);
