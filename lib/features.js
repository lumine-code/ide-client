...
// `symbols` serves every consumer of document and workspace symbols.
exports.FEATURES = [
  "diagnostics",
  "autocomplete",
  "hover",
...
// Which feature governs a request.
exports.METHOD_FEATURES = {
  ...
  // Every spelling is mapped, the way the semantic-token requests are. Only
  // the two `prepare` methods are gated today — nothing calls supports() on a
  // follow-up request — but a switch must never leave half a hierarchy live.
  "textDocument/prepareCallHierarchy": "callHierarchy",
  "callHierarchy/incomingCalls": "callHierarchy",
  "callHierarchy/outgoingCalls": "callHierarchy",
  "textDocument/prepareTypeHierarchy": "typeHierarchy",
  "typeHierarchy/supertypes": "typeHierarchy",
  "typeHierarchy/subtypes": "typeHierarchy",
...
// language, the way a UI package's own setting is.
...
exports.featureEnabled = (adapter, feature, editor) => {
  ...
  if (keyPath) {
    const value = lumine.config.get(`${keyPath}.${feature}`, {
