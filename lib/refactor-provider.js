...
module.exports = class RefactorProvider {
  ...
  async rename(editor, position, newName, { dryRun = false } = {}) {
    ...
    const notebookPaths = this.cellTargetNotebooks(edit);
    if (!this.hasResourceOperations(edit) && !notebookPaths.length)
    ...
    if (dryRun)
      return {
        ...
        // Targets the path map cannot express — notebook cells. The consumer's
        // preview shows the file edits and says the rest is applied hub-side.
        externalTargets: notebookPaths.length > 0,
      };
  ...
  // The notebooks whose cells this edit touches. Cell edits cannot ride the
  // path map — a cell is not a file — so their presence routes the whole edit
  // through the hub's own applyWorkspaceEdit.
  cellTargetNotebooks(edit) {
    const uris = [
    ...
    for (const uri of uris) {
      ...
      if (resolved?.kind === "cell") paths.add(resolved.notebookPath);
    }
    return [...paths];
  }
  ...
  async prepareRename(editor, position) {
    ...
    const session = sessions.find(
      (candidate) =>
        candidate.supports("textDocument/prepareRename", editor) &&
        candidate.capabilityOptions("textDocument/rename", editor)?.prepareProvider,
    );
    if (!session) return null;
