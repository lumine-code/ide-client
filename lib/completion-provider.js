...
module.exports = class CompletionProvider {
  ...
  get triggerCharacters() {
    ...
    for (const session of this.manager.allSessions()) {
      ...
      for (const character of session.capabilityOptions("textDocument/completion")
        ?.triggerCharacters || [])
  ...
  async getSuggestions({ editor, bufferPosition, prefix, activatedManually }) {
    ...
    const responses = await Promise.all(
      sessions.map(async (session) => {
        const provider = session.capabilityOptions("textDocument/completion", editor) || {};
        let context;
  ...
  toSuggestion(session, item) {
    ...
    const editRange =
      edit?.range ||
      (lumine.config.get("autocomplete.consumeSuffix")
  ...
  async getSuggestionDetailsOnSelect(suggestion) {
    const provider = suggestion._lspSession?.capabilityOptions("textDocument/completion");
  ...
  onDidInsertSuggestion({ editor, suggestion }) {
    ...
    if (CLIENT_COMMANDS.has(command.command)) {
      lumine.commands.dispatch(lumine.views.getView(editor), CLIENT_COMMANDS.get(command.command));
      return;
    }
    suggestion._lspSession
      ...
      .catch((error) =>
        lumine.notifications.addWarning(`Language server command failed: ${command.command}`, {
