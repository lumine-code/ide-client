...
module.exports = class LanguageServerManager {
  constructor() {
    ...
    this.workspaceEditPathChangeSuppressions = new Map();
    this.busyProvider = null;
    this.fileOperationsExecutor = null;
    this.fileOperationsLifecycleRegistrations = new Map();
    this.fileOperationsLifecycleExecutor = null;
    this.fileOperationExecutionGates = new Map();
    this.fileOperationBufferedEvents = [];
    this.fileOperationCompletedGates = [];
    this.fileOperationInternalRoots = [];
    this.fileOperationEventSequence = 0;
    this.fileOperationExecutionRequests = [];
    this.fileOperationExecutorIds = new WeakMap();
    this.nextFileOperationExecutorId = 1;
    ...
    this.fileOperationSuppressions = new Map();
  ...
  featureEnabledForPath(adapter, feature, filePath) {
    const scopeName = lumine.grammars.selectGrammar(filePath)?.scopeName;
  ...
  buildClientCapabilities() {
    return mergeCapabilities(
      baseCapabilities({ resourceOperations: !!this.fileOperationsExecutor }),
      ...this.capabilityFragments,
    );
  }
  ...
  disposeRetiredFileOperationsLifecycle(executor) {
    const registration = this.fileOperationsLifecycleRegistrations.get(executor);
    if (!registration?.retiring) return;
    if ([...this.fileOperationExecutionGates.values()].some((gate) => gate.executor === executor)) {
      return;
    }
    registration.subscriptions.dispose();
    this.fileOperationsLifecycleRegistrations.delete(executor);
  }
  retireFileOperationsLifecycle(executor) {
    const registration = this.fileOperationsLifecycleRegistrations.get(executor);
    if (!registration) return;
    registration.retiring = true;
    for (const gate of this.fileOperationExecutionGates.values()) {
    ...
    this.disposeRetiredFileOperationsLifecycle(executor);
  }
  setFileOperationsExecutor(executor) {
    const previous = this.fileOperationsExecutor;
    if (previous && previous !== executor) this.retireFileOperationsLifecycle(previous);
    ...
    this.fileOperationsLifecycleExecutor = executor;
    let registration = this.fileOperationsLifecycleRegistrations.get(executor);
    if (!registration) {
      registration = {
        retiring: false,
        subscriptions: new CompositeDisposable(
          executor.onWillExecuteStep((event) => this.beginFileOperationEventGate(event, executor)),
          executor.onDidExecuteStep((event) => this.finishFileOperationEventGate(event, executor)),
        ),
      };
      this.fileOperationsLifecycleRegistrations.set(executor, registration);
    }
  ...
  watchEditor(editor) {
    ...
    const subs = new CompositeDisposable(
      ...
      editor.onDidChangePath(() => {
        const suppression = this.workspaceEditPathChangeSuppressions.get(editor);
        if (suppression) {
  ...
  routeFileEvents(events) {
    ...
    for (const event of events || []) {
      if (this.fileOperationEventMatchesRoots(event, this.fileOperationInternalRoots)) continue;
      const record = { sequence: ++this.fileOperationEventSequence, event };
      if (this.fileOperationExecutionGates.size) {
        this.fileOperationBufferedEvents.push(record);
      } else {
        visible.push(event);
      }
    }
    if (visible.length) this.deliverFileEvents(visible);
  }
  deliverFileEvents(events) {
    this.notifyWatchedFileEvents(events);

    const fileOperationEvents = events.filter(
      (event) => !this.consumeFileOperationSuppression(event.action, event.path),
    );
    for (const session of this.allSessions()) {
  ...
  notifyWatchedFileEvents(events) {
  ...
  fileOperationEventMatchesRoots(event, roots) {
    return (roots || []).some((root) => {
      if (!root?.path) return false;
      return [event?.path, event?.oldPath].some((filePath) => {
        if (!filePath) return false;
        if (this.workspacePathKey(root.path) === this.workspacePathKey(filePath)) return true;
        return root.recursive && this.relativeWorkspacePath(root.path, filePath) !== null;
      });
    });
  }
  rememberFileOperationInternalRoots(roots) {
    const known = new Set(
      this.fileOperationInternalRoots.map(
        (root) => `${this.workspacePathKey(root.path)}:${root.recursive ? "tree" : "entry"}`,
      ),
    );
    for (const root of roots || []) {
      if (!root?.path) continue;
      const normalized = { path: path.resolve(root.path), recursive: root.recursive === true };
      const key = `${this.workspacePathKey(normalized.path)}:${normalized.recursive ? "tree" : "entry"}`;
      if (known.has(key)) continue;
      known.add(key);
      this.fileOperationInternalRoots.push(normalized);
    }
    if (this.fileOperationInternalRoots.length > 4096) {
      this.fileOperationInternalRoots.splice(0, this.fileOperationInternalRoots.length - 4096);
    }
  }
  fileEventsForEffects(effects) {
    const events = [];
  ...
  fileOperationEventCoveredByGate({ sequence, event }, gate) {
    ...
    return (gate.coveredRoots || []).some(
      (root) =>
        ...
        [event?.path, event?.oldPath].some(
          (filePath) =>
            filePath && this.workspacePathKey(root.path) === this.workspacePathKey(filePath),
        ),
    );
  }
  fileOperationEventGateKey(executor, id) {
    if (!executor || (typeof executor !== "object" && typeof executor !== "function")) {
      return `direct:${id}`;
    }
    let executorId = this.fileOperationExecutorIds.get(executor);
    if (!executorId) {
      executorId = this.nextFileOperationExecutorId++;
      this.fileOperationExecutorIds.set(executor, executorId);
    }
    return `${executorId}:${id}`;
  }
  beginFileOperationEventGate({ id, operation } = {}, executor = null) {
    if (id == null) return;
    const gateKey = this.fileOperationEventGateKey(executor, id);
    if (this.fileOperationExecutionGates.has(gateKey)) return;
    const editorSnapshots = this.editorSnapshotsForFileOperation(operation);
    const pathChangeToken = {};
    this.suppressWorkspaceEditPathChanges(editorSnapshots, pathChangeToken);
    const request = this.fileOperationExecutionRequests.find(
      (candidate) => candidate.executor === executor && candidate.id == null,
    );
    if (request) request.id = id;
    this.fileOperationExecutionGates.set(gateKey, {
      executor,
      startSequence: this.fileOperationEventSequence + 1,
      editorSnapshots,
      pathChangeToken,
      request,
    });
  }
  async finishFileOperationEventGate({ id, result, eventTrace } = {}, executor = null) {
    const gateKey = this.fileOperationEventGateKey(executor, id);
    const gate = this.fileOperationExecutionGates.get(gateKey);
    if (!gate) return;
    let operationError;
    let logicalEditorPaths;
    try {
      try {
        logicalEditorPaths = this.updateEditorsForFileEffects(
          result?.effects,
      ...
      await this.stabilizeWorkspaceEditEditorPaths(logicalEditorPaths);
      this.rememberFileOperationInternalRoots(eventTrace?.internalRoots);
      this.fileOperationCompletedGates.push({
        startSequence: gate.startSequence,
        endSequence: this.fileOperationEventSequence,
        coveredRoots: eventTrace?.coveredRoots || [],
      });
      try {
        this.publishFileOperationEffects(result?.effects, { suppressProjectEvents: false });
        this.notifyWatchedFileEvents(this.fileEventsForEffects(result?.effects));
      } catch (error) {
        operationError ||= error;
      }
      await this.reattachWorkspaceEditPathChanges(gate.pathChangeToken);
    } finally {
      this.releaseWorkspaceEditPathChanges(gate.pathChangeToken);
      this.fileOperationExecutionGates.delete(gateKey);
      this.disposeRetiredFileOperationsLifecycle(gate.executor);
      if (!this.fileOperationExecutionGates.size) this.flushFileOperationEventGates();
    }
    if (operationError) {
      if (gate.request) gate.request.error = operationError;
      else
        console.error("ide-client: file-operation lifecycle coordination failed", operationError);
    }
  }
  flushFileOperationEventGates() {
    for (const gate of this.fileOperationExecutionGates.values()) {
    ...
    this.fileOperationExecutionGates.clear();
    const completedGates = this.fileOperationCompletedGates;
    this.fileOperationCompletedGates = [];
    const events = this.fileOperationBufferedEvents
      .filter(
        (record) =>
          !this.fileOperationEventMatchesRoots(record.event, this.fileOperationInternalRoots) &&
          !completedGates.some((gate) => this.fileOperationEventCoveredByGate(record, gate)),
      )
      .map(({ event }) => event);
    ...
    this.fileOperationBufferedEvents = [];
    if (events.length) this.deliverFileEvents(events);
  }
  fileOperationMatches(filters, filePath, isDirectory) {
  ...
  suppressProjectFileEvent(action, filePath) {
    const key = `${action}:${C.uriKey(C.pathToUri(filePath))}`;
    this.fileOperationSuppressions.set(key, Date.now() + 5000);
  }
  consumeFileOperationSuppression(action, filePath) {
    const now = Date.now();
    for (const [key, expires] of this.fileOperationSuppressions)
      if (expires <= now) this.fileOperationSuppressions.delete(key);
    const key = `${action}:${C.uriKey(C.pathToUri(filePath))}`;
    if (!this.fileOperationSuppressions.has(key)) return false;
    this.fileOperationSuppressions.delete(key);
    return true;
  }
  ...
  matchingFileEntries(session, capability, entries) {
    ...
    return entries.filter((entry) =>
      this.fileOperationMatches(filters, entry.path, entry.isDirectory),
    );
  }
  matchingRenameEntries(session, capability, entries) {
    ...
    return entries.filter(
      (entry) =>
        this.fileOperationMatches(filters, entry.oldPath, entry.isDirectory) ||
        this.fileOperationMatches(filters, entry.newPath, entry.isDirectory),
    );
  }
  async willFileOperation(method, capability, entries, paramsFor, label) {
    const edits = [];
    for (const session of this.allSessions()) {
      if (session.state !== "running") continue;
      ...
      try {
        const edit = await session.request(method, paramsFor(matched));
        if (edit) edits.push({ edit, session });
      } catch (error) {
        this.log(session, `${method} failed: ${error.message}`);
        lumine.notifications.addWarning(
          `${serverName(session)} could not prepare the file operation`,
          {
            detail: error.message,
            dismissable: true,
          },
    ...
    return this.applyWorkspaceEdits(edits, label);
  }
  willCreateFiles(payload) {
    return this.willFileOperation(
  ...
  didCreateFiles(payload, { excludeSession = null, suppressProjectEvents = true } = {}) {
    ...
    if (suppressProjectEvents)
      for (const entry of entries) this.suppressProjectFileEvent("created", entry.path);
    this.notifyFileOperation(
  ...
  didRenameFiles(payload, { excludeSession = null, suppressProjectEvents = true } = {}) {
    ...
    if (suppressProjectEvents)
      for (const entry of entries) {
        this.suppressProjectFileEvent("deleted", entry.oldPath);
        this.suppressProjectFileEvent("created", entry.newPath);
      }
    this.notifyFileOperation(
  ...
  didDeleteFiles(payload, { excludeSession = null, suppressProjectEvents = true } = {}) {
    const entries = this.fileEntries(payload);
    if (suppressProjectEvents)
      for (const entry of entries) this.suppressProjectFileEvent("deleted", entry.path);
    this.notifyFileOperation(
  ...
  notifyFileOperation(method, capability, entries, paramsFor, { excludeSession = null } = {}) {
    for (const session of this.allSessions()) {
  ...
  disposeFileOperationPlan(plan) {
    try {
      ...
      console.error("ide-client: failed to dispose a file-operation plan", error);
    }
  }
  async prepareFileOperations(documentChanges) {
    const executor = this.fileOperationsExecutor;
    const indexed = [];
    for (
    ...
    let prepared;
    try {
      prepared = await executor.prepare(indexed.map(({ operation }) => operation));
    } catch (error) {
  ...
  async assertWorkspaceEditPathExists(executor, filePath, uri) {
    if (typeof executor?.inspect !== "function") return;
    const descriptions = await executor.inspect([filePath]);
  ...
  relativeWorkspacePath(parentPath, candidatePath) {
    if (this.workspacePathKey(parentPath) === this.workspacePathKey(candidatePath)) return "";
    ...
    for (let index = resources.length - 1; index >= 0; index--) {
      ...
      if (plannedStatus === "skip") {
        if (
          ...
          this.relativeWorkspacePath(operation.path, projected) !== null
        ) {
      ...
      if (operation.kind === "create") {
        if (this.relativeWorkspacePath(operation.path, projected) !== null) {
          return { status: "new", path: projected };
        }
        continue;
      }
      if (operation.kind === "rename") {
        const targetRelative = this.relativeWorkspacePath(operation.newPath, projected);
        if (targetRelative !== null) {
          projected = path.join(operation.oldPath, targetRelative);
          continue;
        }
        if (this.relativeWorkspacePath(operation.oldPath, projected) !== null) {
          return { status: "missing", path: projected };
        }
        continue;
      }
  ...
  editorsWithinWorkspacePath(filePath) {
    return lumine.workspace
      .getTextEditors()
      .filter(
        (editor) =>
          editor.getPath?.() && this.relativeWorkspacePath(filePath, editor.getPath()) !== null,
      );
  }
  editorSnapshotsForFileOperation(operation) {
    const roots = [];
    ...
    return lumine.workspace
      ...
      .filter(
        (snapshot) =>
          snapshot.path &&
          roots.some((root) => root && this.relativeWorkspacePath(root, snapshot.path) !== null),
      );
  }
  suppressWorkspaceEditPathChanges(snapshots, token) {
    for (const { editor } of snapshots) {
      let suppression = this.workspaceEditPathChangeSuppressions.get(editor);
      if (!suppression) {
        suppression = { tokens: new Set(), changed: false };
        this.workspaceEditPathChangeSuppressions.set(editor, suppression);
      }
      suppression.tokens.add(token);
    }
  }
  async stabilizeWorkspaceEditEditorPaths(logicalPaths) {
    await Promise.allSettled(
      [...logicalPaths].map(async ([editor, logicalPath]) => {
        const buffer = editor.getBuffer?.();
        if (!buffer) return;
        for (let attempt = 0; attempt < 4 && !editor.isDestroyed?.(); attempt++) {
          if (editor.getPath?.() !== logicalPath) buffer.setPath(logicalPath);
          const watchStart = buffer.getFileWatchStartPromise?.() || Promise.resolve();
          await watchStart;
          if (
            editor.getPath?.() === logicalPath &&
            watchStart === (buffer.getFileWatchStartPromise?.() || watchStart)
          ) {
            return;
          }
        }
        ...
        if (editor.getPath?.() !== logicalPath) buffer.setPath(logicalPath);
        // Always await the latest promise after exhausting the identity loop:
        // it may have changed on the last pass even when the path was already
        // logical and therefore needed no final setPath().
        await (buffer.getFileWatchStartPromise?.() || Promise.resolve());
        if (editor.getPath?.() !== logicalPath) {
          buffer.setPath(logicalPath);
          await (buffer.getFileWatchStartPromise?.() || Promise.resolve());
        }
      }),
    );
  }
  async reattachWorkspaceEditPathChanges(token) {
    const reattachments = [];
    for (const [editor, suppression] of this.workspaceEditPathChangeSuppressions) {
  ...
  releaseWorkspaceEditPathChanges(token) {
    for (const [editor, suppression] of this.workspaceEditPathChangeSuppressions) {
      if (!suppression.tokens.delete(token) || suppression.tokens.size) continue;
      this.workspaceEditPathChangeSuppressions.delete(editor);
    }
  }
  assertFileOperationTargetIsClosed(resource, documentChangeIndex) {
    ...
    if (
      ...
      this.workspacePathKey(operation.oldPath) === this.workspacePathKey(operation.newPath)
    ) {
      return;
    }
    if (this.editorsWithinWorkspacePath(target).length) {
      throw this.workspaceEditError(
  ...
  async workspaceEditShapeError(documentChanges, session, executor) {
        ...
        const touchedByResourceOperation =
          ...
          precedingFileOperations.some((candidate) => {
            if (candidate.kind === "rename") {
              return (
                this.relativeWorkspacePath(candidate.oldPath, resolved.path) !== null ||
                this.relativeWorkspacePath(candidate.newPath, resolved.path) !== null
              );
            }
            return this.relativeWorkspacePath(candidate.path, resolved.path) !== null;
          });
          ...
          const openEditor =
            ...
            lumine.workspace
              ...
              .some(
                (editor) =>
                  editor.getPath?.() &&
                  this.workspacePathKey(editor.getPath()) === this.workspacePathKey(resolved.path),
              );
          if (resolved.kind === "file" && !document && !openEditor) {
            await this.assertWorkspaceEditPathExists(executor, resolved.path, uri);
          }
          if (expected != null) {
            if (!document)
  ...
  async preflightWorkspaceEdit(documentChanges, session) {
    const steps = [];
    const versionStates = new Map();
    ...
    try {
      [shapeError, filePreparation] = await Promise.all([
        shapeErrorPromise,
        this.prepareFileOperations(documentChanges),
      ]);
    } catch (error) {
    ...
    try {
      for (
        ...
        try {
          ...
          const openEditor =
            ...
            lumine.workspace
              ...
              .find(
                (candidate) =>
                  candidate.getPath?.() &&
                  this.workspacePathKey(candidate.getPath()) ===
                    this.workspacePathKey(projected.path),
              );
          if (
            resolved.kind === "file" &&
            ...
            await this.assertWorkspaceEditPathExists(filePreparation.executor, projected.path, uri);
          }
          ...
          const key = C.uriKey(uri);
          const deferred = resolved.kind === "file" && projected.status === "new";
          let editor = deferred ? null : editors.get(key);
          if (!deferred && !editor) {
            const editorUri =
              resolved.kind === "file" && projected.path !== resolved.path
                ? C.pathToUri(projected.path)
                : uri;
            editor = await this.editorForWorkspaceEdit(editorUri);
  ...
  workspaceEditPlanIsCurrent(plan) {
    return !this.staleWorkspaceEditStep(plan);
  }
  ...
  workspaceEditDocumentChanges(edit) {
    ...
    if (edit.changes !== undefined && (!edit.changes || typeof edit.changes !== "object")) {
  ...
  async applyWorkspaceEdits(edits, label) {
    const changes = [];
    let plan;
    try {
      for (const { edit, session } of edits)
        for (const change of this.workspaceEditDocumentChanges(edit))
          changes.push({ ...change, sourceSession: session });
      plan = await this.preflightWorkspaceEdit(changes, null);
    } catch (error) {
    ...
    if (plan.steps.every((step) => step.kind === "text")) {
      try {
        ...
        for (const step of plan.steps) {
          const editor =
            step.editor || (await this.editorForWorkspaceEdit(step.change.textDocument.uri));
          ...
          for (const textEdit of step.change.edits) {
            ...
            if (!duplicate) list.push({ range: textEdit.range, newText });
          }
        ...
        for (const editsForEditor of byEditor.values()) {
          ...
          for (let index = 1; index < ascending.length; index++)
            if (this.workspaceTextEditsOverlap(ascending[index - 1], ascending[index]))
              throw new Error("Language servers returned overlapping file-operation edits");
        }
        for (const [editor, editsForEditor] of byEditor) {
          editor.transact(() => {
            for (const textEdit of [...editsForEditor].sort((a, b) =>
              this.compareLspPositions(b.range.start, a.range.start),
            ))
    ...
    return this.applyWorkspaceEditPlan(plan, label);
  }
  ...
  async applyWorkspaceEditDetailed(edit, label, session = null) {
    ...
    let documentChanges;
    try {
      documentChanges = this.workspaceEditDocumentChanges(edit);
    } catch (error) {
  ...
  async applyWorkspaceEditPlan(plan, label) {
    return (await this.applyWorkspaceEditPlanDetailed(plan, label)).applied;
  }
  updateEditorsForFileEffects(effects, snapshots = []) {
    const logicalPaths = new Map(snapshots.map(({ editor, path: filePath }) => [editor, filePath]));
    for (const effect of effects || []) {
      if (effect?.kind !== "rename" || !effect.oldPath || !effect.newPath) continue;
      for (const [editor, editorPath] of logicalPaths) {
        if (!editorPath || editor.isDestroyed?.()) continue;
        const relative = this.relativeWorkspacePath(effect.oldPath, editorPath);
        if (relative === null) continue;
        logicalPaths.set(editor, path.join(effect.newPath, relative));
      }
    }
    // Reconcile every snapshot, including a failed operation with no effects:
    // the executor may have moved a path through a private tombstone and rolled
    // it back before reporting the logical result.
    for (const [editor, logicalPath] of logicalPaths) {
      if (!editor.isDestroyed?.() && editor.getPath?.() !== logicalPath) {
        editor.getBuffer().setPath(logicalPath);
      }
    }
    return logicalPaths;
  }
  publishFileOperationEffects(effects, { suppressProjectEvents = true } = {}) {
    const creates = [];
    const renames = [];
    const deletes = [];
    for (const effect of effects || []) {
  ...
  fileOperationCoordinationError(error, result) {
    const paths = this.fileOperationCleanupPaths(result);
  ...
  async executeWorkspaceFileStep(plan, step) {
    if (plan.fileOperationsExecutor === this.fileOperationsLifecycleExecutor) {
      const request = { executor: plan.fileOperationsExecutor, id: null, error: null };
      this.fileOperationExecutionRequests.push(request);
      try {
        const result = await plan.filePlan.executeNext();
        if (request.error) throw this.fileOperationCoordinationError(request.error, result);
        return result;
      } finally {
        const index = this.fileOperationExecutionRequests.indexOf(request);
        if (index >= 0) this.fileOperationExecutionRequests.splice(index, 1);
      }
    }
    const editorSnapshots = this.editorSnapshotsForFileOperation(step.operation);
    const pathChangeToken = {};
    this.suppressWorkspaceEditPathChanges(editorSnapshots, pathChangeToken);
    let result;
    let operationError;
    ...
    try {
      try {
        result = await plan.filePlan.executeNext();
      } catch (error) {
        operationError = error;
      }
      try {
        logicalEditorPaths = this.updateEditorsForFileEffects(result?.effects, editorSnapshots);
      } catch (error) {
        operationError ||= error;
      }
    } finally {
      ...
      try {
        if (result) this.publishFileOperationEffects(result.effects);
      } catch (error) {
        operationError ||= error;
      }
      try {
        await this.reattachWorkspaceEditPathChanges(pathChangeToken);
        await this.stabilizeWorkspaceEditEditorPaths(logicalEditorPaths);
      } finally {
        this.releaseWorkspaceEditPathChanges(pathChangeToken);
      }
    }
    if (operationError) throw this.fileOperationCoordinationError(operationError, result);
    return result;
  }
  async applyWorkspaceEditPlanDetailed(plan, label, { includeFailedChange = true } = {}) {
    ...
    try {
      ...
      for (const step of plan.steps) {
        ...
        if (step.kind === "resource") {
          if (
            !this.fileOperationsExecutor ||
            this.fileOperationsExecutor !== plan.fileOperationsExecutor
          ) {
            throw this.workspaceEditError(
          ...
          let result;
          try {
            result = await this.executeWorkspaceFileStep(plan, step);
          } catch (error) {
        ...
        try {
          if (step.expectedVersion != null) {
            const state = step.documentState;
            if (!state || this.workspaceEditLogicalVersion(state) !== step.expectedVersion) {
              throw new Error(
                `Refusing a stale workspace edit for '${change.textDocument.uri}': document version changed`,
              );
            }
          }
          const editor =
            step.editor || (await this.editorForWorkspaceEdit(change.textDocument.uri));
          editor.transact(() =>
            [...change.edits]
              .sort(
                (a, b) =>
  ...
  async drainControllerRestart(controller) {
    while (this.controllerIsActive(controller)) {
      ...
      if (current) {
        ...
        if (stopError) {
          const exited = this.sessionPhysicallyExited(current);
          if (stopError.exitNotificationTimedOut && exited) {
            this.log(current, `Exit notification timed out; continuing restart after process exit`);
          } else {
            if (stale && exited) continue;
            if (!exited) this.blockControllerOnLiveSession(controller, current);
            throw stopError;
          }
        }
        if (stale) continue;
  ...
  async deactivate() {
    ...
    for (const registration of this.fileOperationsLifecycleRegistrations.values()) {
      registration.subscriptions.dispose();
    }
    this.fileOperationsLifecycleRegistrations.clear();
    this.fileOperationsLifecycleExecutor = null;
    this.flushFileOperationEventGates();
