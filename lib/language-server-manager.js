...
module.exports = class LanguageServerManager {
  ...
  async prepareSessionServices() {
    const requests = [];
    if (!this.fileOperationsExecutor) {
  ...
  publishSession(controller, session) {
    ...
    controller.degradedObservation = null;
    controller.session = session;
  ...
  projectRootsForFolders(folders, projectPaths = lumine.project.getPaths()) {
    return projectPaths.filter((root) =>
      folders.some(
        (folder) =>
          this.relativeWorkspacePath(root, folder) !== null ||
          this.relativeWorkspacePath(folder, root) !== null,
      ),
    );
  }
  ...
  async awaitProjectWatching(controller, folders, generation, revision) {
    ...
    let timer;
    try {
      const observed = await this.waitForControllerChange(
        controller,
        generation,
        revision,
        Promise.race([
      ...
      record.ready
        .then(async (handle) => {
          ...
          const wait = async (promise) => {
            let result;
            do {
              result = await this.waitForControllerChange(
                controller,
                generation,
                controller.revision,
                promise,
  ...
  async ensureSession(adapter, rootPath, { filePath } = {}) {
    ...
    const pending = (async () => {
      while (this.controllerIsActive(controller)) {
        ...
        this.recoverDegradedObservation(controller, replacement, prepared.observations);
        return replacement;
      }
      return null;
    })();
  ...
  assertFileOperationTargetIsClosed(resource, documentChangeIndex) {
    ...
    if (
      lumine.workspace
        .getPaneItems()
        .some(
          (item) => item.getPath?.() && this.relativeWorkspacePath(target, item.getPath()) !== null,
        )
    ) {
      throw this.workspaceEditError(
  ...
  updateEditorsForFileEffects(_effects, snapshots = []) {
        ...
        ? lumine.workspace.beginFileMove([
            {
              ...
              isDirectory:
                require("node:fs")
                  .lstatSync(step.operation.oldPath, { throwIfNoEntry: false })
                  ?.isDirectory() === true,
            },
          ])
        : null;
