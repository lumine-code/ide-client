    ...
    try {
      for (const change of documentChanges) {
        ...
        if (change.kind === "rename") {
          ...
          if (fs.existsSync(target) && !sameEntry) {
        ...
        if (!this.versionedDocumentMatches(change, session))
          throw new Error(
            `Refusing a stale workspace edit for '${change.textDocument?.uri}': document version changed`,
          );
        const editor = await this.editorForWorkspaceEdit(change.textDocument?.uri);
        editor.transact(() =>
          [...change.edits]
            .sort(
              (a, b) =>
