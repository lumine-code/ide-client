...
function managedServerFaults(descriptor) {
  ...
  if (descriptor.source === "npm") {
    ...
    if (!descriptor.binary) faults.push(named("binary"));
...
module.exports = class LanguageServerManager {
  ...
  registerAdapter(adapter) {
    ...
    faults.push(...managedServerFaults(adapter?.managedServer));
  ...
  publishDiagnostics(session, params) {
    ...
    byUri.set(C.uriKey(params.uri), { session, ...params });
  ...
  async showDocument({ uri, selection, external, takeFocus }) {
    try {
      ...
      const editor = await lumine.workspace.open(filePath, { activateItem: takeFocus !== false });
  ...
  async applyWorkspaceEdit(edit, label, session = null) {
    ...
    try {
      for (const change of documentChanges) {
        ...
        if (!filePath) continue;
        const editor =
          lumine.workspace.getTextEditors().find((item) => item.getPath() === filePath) ||
          (await lumine.workspace.open(filePath, { activateItem: false, pending: true }));
