...
module.exports = class LanguageServerManager {
  ...
  didCreateFiles(payload) {
    const entries = this.fileEntries(payload);
    for (const entry of entries) this.suppressProjectFileEvent("created", entry.path);
    this.notifyFileOperation("workspace/didCreateFiles", "didCreate", entries, (matched) => ({
      files: matched.map((entry) => ({ uri: C.pathToUri(entry.path) })),
    }));
  }
  didRenameFiles(payload) {
    ...
    this.notifyFileOperation("workspace/didRenameFiles", "didRename", entries, (matched) => ({
  ...
  didDeleteFiles(payload) {
    const entries = this.fileEntries(payload);
    for (const entry of entries) this.suppressProjectFileEvent("deleted", entry.path);
    this.notifyFileOperation("workspace/didDeleteFiles", "didDelete", entries, (matched) => ({
      files: matched.map((entry) => ({ uri: C.pathToUri(entry.path) })),
    }));
  }
  notifyFileOperation(method, capability, entries, paramsFor) {
    for (const session of this.allSessions()) {
      if (session.state !== "running") continue;
      const matched = capability.includes("Rename")
        ? this.matchingRenameEntries(session, capability, entries)
  ...
  pathsReferToSameEntry(left, right) {
  ...
  async preflightWorkspaceEdit(documentChanges, session) {
    ...
    const materializedDuringPlan = new Set();
    ...
    const exists = (filePath) => {
      ...
      return pathStates.has(key) ? pathStates.get(key) : fs.existsSync(filePath);
    };
    const setExists = (filePath, value) => pathStates.set(pathKey(filePath), value);

    for (const change of documentChanges) {
      const sourceSession = change.sourceSession || session;
      if (change.kind === "create") {
        const target = this.pathForResourceOperation(change.uri);
        const targetExists = exists(target);
        const skip = targetExists && !change.options?.overwrite && change.options?.ignoreIfExists;
        if (targetExists && !change.options?.overwrite && !skip)
          throw new Error(`Cannot create '${target}': it already exists`);
        setExists(target, true);
        if (!skip) materializedDuringPlan.add(pathKey(target));
        steps.push({ kind: "create", change, target, skip, session: sourceSession });
        continue;
      }
      if (change.kind === "rename") {
        const source = this.pathForResourceOperation(change.oldUri);
        const target = this.pathForResourceOperation(change.newUri);
        const sameEntry = this.pathsReferToSameEntry(source, target);
        if (!exists(source)) throw new Error(`Cannot rename '${source}': it does not exist`);
        const targetExists = exists(target) && !sameEntry;
        const skip = targetExists && !change.options?.overwrite && change.options?.ignoreIfExists;
        if (targetExists && !change.options?.overwrite && !skip)
          throw new Error(`Cannot rename to '${target}': it already exists`);
        if (!skip) {
          setExists(source, false);
          setExists(target, true);
          materializedDuringPlan.add(pathKey(target));
        }
        steps.push({
          kind: "rename",
          change,
      ...
      if (change.kind === "delete") {
        const target = this.pathForResourceOperation(change.uri);
        const targetExists = exists(target);
        const skip = !targetExists && change.options?.ignoreIfNotExists;
        if (!targetExists && !skip) throw new Error(`Cannot delete '${target}': it does not exist`);
        if (!skip) setExists(target, false);
        steps.push({ kind: "delete", change, target, skip, session: sourceSession });
        continue;
      }
      ...
      if (expected != null) {
        if (!document) throw new Error(`Cannot validate versioned workspace edit target '${uri}'`);
        const current = versionStates.has(document)
          ? versionStates.get(document)
          : document.version;
        if (!baselineVersions.has(document)) baselineVersions.set(document, document.version);
        if (current !== expected)
          throw new Error(`Refusing a stale workspace edit for '${uri}': document version changed`);
      ...
      if (!resolved) throw new Error(`Cannot resolve workspace edit target '${uri}'`);
      const deferred =
        resolved.kind === "file" && materializedDuringPlan.has(pathKey(resolved.path));
      let editor = deferred ? null : editors.get(key);
      if (!deferred && !editor) {
        editor = await this.editorForWorkspaceEdit(uri);
        editors.set(key, editor);
      }
      steps.push({ kind: "text", change, editor, deferred, session: sourceSession });
    }
    return { steps, baselineVersions };
  }
  workspaceEditPlanIsCurrent(plan) {
    for (const [document, version] of plan.baselineVersions)
  ...
  async applyWorkspaceEdit(edit, label, session = null) {
    const documentChanges = this.workspaceEditDocumentChanges(edit);
    let plan;
    try {
  ...
  async applyWorkspaceEditPlan(plan, label) {
    const destructive = plan.steps.some(
      (step) => !step.skip && (step.kind === "delete" || step.kind === "rename"),
    );
    if (destructive) {
    ...
    try {
      for (const step of plan.steps) {
        ...
        if (step.kind === "rename") {
          const { source, target, targetExists } = step;
        ...
        if (step.kind === "delete") {
          ...
          try {
            ...
            if (change.options?.ignoreIfNotExists && error.code === "ENOENT") continue;
            throw error;
          }
          // fs.rm refuses every directory when recursive is false, including
          // an empty one. LSP DeleteFile only needs recursive for contents: an
          // empty directory is a valid non-recursive deletion.
          if (stats.isDirectory() && !change.options?.recursive) {
        ...
        const editor = step.editor || (await this.editorForWorkspaceEdit(change.textDocument.uri));
        editor.transact(() =>
          [...change.edits]
            .sort(
              (a, b) =>
