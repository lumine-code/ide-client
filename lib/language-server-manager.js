...
module.exports = class LanguageServerManager {
  ...
  activate() {
    ...
    this.subscriptions.add(
      ...
      atom.window.onWillDestroy(() => this.killAllSessions()),
    );
  }
  ...
  registerAdapter(adapter) {
    ...
    if (features)
      subs.add(
        atom.config.onDidChange(features, () =>
  ...
  async showMessageRequest(type, message, actions) {
    ...
    const selected = await atom.window.confirm({
  ...
  async applyWorkspaceEdit(edit, label, session = null) {
    ...
    if (destructive) {
      const choice = await atom.window.confirm({
        type: "warning",
