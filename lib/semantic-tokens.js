...
module.exports = class SemanticTokens {
  ...
  constructor(manager, tracker) {
    ...
    this.subscriptions = new CompositeDisposable(
      ...
      lumine.config.onDidChange("ide-client.semanticTokens.enabled", () => this.fetchAll()),
  ...
  enabledFor(editor) {
    return !!lumine.config.get("ide-client.semanticTokens.enabled", {
  ...
  async fetch(state) {
    ...
    const provider = session.capabilityOptions("textDocument/semanticTokens", editor);
    ...
    const textDocument = { uri: this.manager.uriForEditor(editor) };
    let result;
    try {
