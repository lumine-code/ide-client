...
module.exports = class DocumentFeatures {
  ...
  async requestForCommand(session, method, params) {
    try {
  ...
  async foldRanges(editor) {
    ...
    const result = await this.requestForCommand(session, "textDocument/foldingRange", {
      textDocument: { uri: this.manager.uriForEditor(editor) },
    });
    if (result === REQUEST_FAILED) return false;
  ...
  async expandSelectionRanges(editor) {
    ...
    const result = await this.requestForCommand(session, "textDocument/selectionRange", {
      textDocument: { uri: this.manager.uriForEditor(editor) },
      positions,
    });
    if (result === REQUEST_FAILED) return false;
  ...
  async selectLinkedRanges(editor) {
    ...
    if (!session) return false;
    const result = await this.requestForCommand(session, "textDocument/linkedEditingRange", {
      textDocument: { uri: this.manager.uriForEditor(editor) },
  ...
  async colorPresentations(editor) {
    ...
    const colors = await this.requestForCommand(session, "textDocument/documentColor", {
      textDocument: { uri },
    });
    if (colors === REQUEST_FAILED) return false;
    ...
    const presentations = await this.requestForCommand(session, "textDocument/colorPresentation", {
      textDocument: { uri },
      color: color.color,
      range: color.range,
    });
    if (presentations === REQUEST_FAILED) return false;
