...
module.exports = class SessionMenuView {
  constructor(main) {
    ...
    this.serverList = lumine.workspace.buildSelectList({
      ...
      confirmAction: "ide-client:show-details",
    ...
    this.detailsList = lumine.workspace.buildSelectList({
    ...
    this.subscriptions.add(
      ...
      // item-actions list derives its rows — label, description,
      ...
      // `main.js` registers on `lumine-workspace`, which the actions list filters
      // out as chrome from above.
      lumine.commands.add(this.serverList.element, {
  ...
  copyDetail(item) {
    ...
    return this.detailsList.update({
      status: { message: `Copied ${item.label}`, duration: 2000 },
    });
  }
