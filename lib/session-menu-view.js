...
module.exports = class SessionMenuView {
  constructor(main) {
    ...
    this.subscriptions.add(
      ...
      this.serverList.onDidResume(() => this.serverList.setItems(this.serverItems())),
    );
  }
  ...
  async showDetails(session) {
    ...
    return this.detailsList.show({ crumb: session.adapter.displayName });
  }
  ...
  async toggle() {
    ...
    return this.serverList.show();
  }

  destroy() {
    this.subscriptions.dispose();
    return Promise.all([this.serverList.destroy(), this.detailsList.destroy()]);
  }
};
