...
module.exports = class InlayHints {
  ...
  constructor(manager, tracker) {
    ...
    this.subscriptions = new CompositeDisposable(
      ...
      lumine.config.onDidChange("ide-client.inlayHints.enabled", () => this.fetchAll()),
      lumine.config.onDidChange("ide-client.inlayHints.maxLabelLength", () => this.fetchAll()),
      // Which server answers can change without any server changing.
      manager.onDidChangeFeatures(() => this.fetchAll()),
      // A capability registered after the session came up was absent when this
      // last looked, so it concluded the server could not serve it.
      manager.onDidChangeCapabilities(({ session }) => this.refreshSession(session)),
    );
  }
  watchEditor(editor) {
  ...
  enabledFor(editor) {
    return !!lumine.config.get("ide-client.inlayHints.enabled", {
  ...
  labelFor(hint) {
    const label = Array.isArray(hint.label)
      ...
      : hint.label || "";
    const max = lumine.config.get("ide-client.inlayHints.maxLabelLength") ?? 48;
