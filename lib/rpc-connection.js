...
const FileCancellationSender = require("./file-cancellation-sender");

const abortReason = (signal) => signal.reason || new Error("Request cancelled");

// vscode-jsonrpc 9.0.2 rejects the request promise when a write fails, then
// throws the same error from an async Promise executor. The first rejection is
// observable by the caller; the second has no owner and reaches the renderer as
// `Uncaught (in promise)`. Keep request-write failures on the connection's
// ordinary error channel and dispose it there, which rejects every pending
// request without letting the writer rejection enter that broken branch.
class RequestSafeMessageWriter {
  constructor(writer, onRequestWriteFailure) {
    this.writer = writer;
    this.onRequestWriteFailure = onRequestWriteFailure;
  }
  get onError() {
    return this.writer.onError;
  }
  get onClose() {
    return this.writer.onClose;
  }
  write(message) {
    try {
      return Promise.resolve(this.writer.write(message)).catch((error) =>
        this.handleWriteFailure(message, error),
      );
    } catch (error) {
      return this.handleWriteFailure(message, error);
    }
  }
  handleWriteFailure(message, error) {
    const request = typeof message?.method === "string" && Object.hasOwn(message, "id");
    if (!request) throw error;
    this.onRequestWriteFailure(error);
  }
  end() {
...
module.exports = class RpcConnection {
  ...
  constructor(reader, writer, { logger, fileCancellationFolder } = {}) {
    this.logger = logger;
    this.disposed = false;
    this.cancellationSender = fileCancellationFolder
      ? new FileCancellationSender(fileCancellationFolder)
      : null;
    const options = this.cancellationSender
      ? {
          cancellationStrategy: {
            receiver: CancellationStrategy.Message.receiver,
            sender: this.cancellationSender,
          },
        }
      : undefined;
    // The public constructor also accepts a raw Node stream; the static
    // transport constructors arrive with an RPC writer already.
    if (typeof writer.onError !== "function") writer = new StreamMessageWriter(writer);
    const safeWriter = new RequestSafeMessageWriter(writer, () => this.dispose());
    this.connection = createMessageConnection(reader, safeWriter, logger, options);
  }
  ...
  // Catch-all for server-specific requests that an adapter has opted into.
  // Named core handlers still take precedence inside vscode-jsonrpc.
  onOtherRequest(handler) {
    return this.connection.onRequest(handler);
  }
  ...
  dispose() {
    ...
    try {
      ...
      this.cancellationSender?.dispose();
    }
  }
};
