...
module.exports = class ManagedServersView {
  constructor(main) {
    ...
    this.list = lumine.workspace.buildSelectList({
      ...
      confirmAction: (item) => {
        if (!item) return null;
        return item.entry.installed ? "ide-client:update-server" : "ide-client:install-server";
      },
      actionsFilter: ({ name }) => this.actionAppliesToSelection(name),
  ...
  actionAppliesToSelection(command) {
    if (command === "ide-client:check-server-updates") return true;
    const entry = this.selected();
    if (!entry) return false;
    if (command === "ide-client:install-server") return !entry.installed;
    if (command === "ide-client:update-server" || command === "ide-client:uninstall-server") {
