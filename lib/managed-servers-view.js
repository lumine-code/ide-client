...
module.exports = class ManagedServersView {
  constructor(main) {
    ...
    this.listHost = lumine.workspace.addSelectList(
      {
        items: [],
    ...
    this.list = this.listHost.getModel();
  ...
  async refresh() {
    if (!this.listHost.isVisible()) return;
    await this.list.setItems(this.items());
  }

  async toggle() {
    if (this.listHost.isVisible()) return this.listHost.hide();
    return this.listHost.show();
  }

  destroy() {
    this.subscriptions.dispose();
    return this.listHost.destroy();
  }
};
