...
//
//   github-release — a per-platform archive attached to a GitHub release. This
//     is the only way to get ruff, texlab or tinymist: they are Rust binaries
//     with no server at all until one is fetched.
...
function compareVersions(a, b) {
  const parse = (value) => {
    const [core, prerelease] = String(value ?? "")
      .replace(/^v/, "")
      .split("-");
    return {
...
module.exports = class ManagedServers {
  ...
  async stageGithubRelease(adapter, stage, resolved) {
    ...
    const archive = await this.download(url, `${descriptor.displayName} ${resolved.version}`);
    await this.verify(archive, url, descriptor.checksum);
  ...
  extract(archivePath, destination, archiveName, strip = 0) {
  ...
  extractZip(archivePath, destination, strip) {
