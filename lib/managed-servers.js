...
module.exports = class ManagedServers {
  ...
  describe(adapter) {
    ...
    return {
      adapter,
      displayName: descriptor.displayName || adapter.displayName,
  ...
  async npmMetadata(name, version) {
    const response = await this.fetchUrl(`https://registry.npmjs.org/${name}/${version}`);
    if (!response.ok)
  ...
  async stageFromAdapter(adapter, stage, resolved) {
    ...
    if (!returned?.binary && !returned?.module)
      throw new Error(
        `${adapter.displayName} installed its server but did not say which file to launch; installServer must return a 'binary' or a 'module' path.`,
      );
    return {
      source: "adapter",
