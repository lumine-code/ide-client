...
module.exports = class ServerStatusView {
  constructor({ manager, onDidClick }) {
    ...
    this.element.classList.add("ide-client-server-status");
    ...
    this.subscriptions.add(
      ...
      lumine.config.observe("ide-client.statusBar.enabled", (enabled) => {
  ...
  render(sessions) {
    ...
    //
    // A status-bar tile is a flex item of its panel, so it has a display from
    // the stylesheet either way, and that outranks the hidden attribute:
    // visibility goes through the style attribute instead.
