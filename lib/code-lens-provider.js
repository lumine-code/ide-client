...
module.exports = class CodeLensProvider {
  ...
  async codeLenses(editor) {
    ...
    const result = (lenses || []).map((lens) => this.toCodeLens(lens, session)).filter(Boolean);
  ...
  async resolveCodeLens(lens) {
    ...
    const { lsp, session } = source;
    if (!session.capabilities?.codeLensProvider?.resolveProvider) return null;
    ...
    return this.toCodeLens({ ...resolved, range: resolved.range || lsp.range }, session);
  }
  toCodeLens(lsp, session) {
    ...
    this.sources.set(lens, { lsp, session });
    return lens;
  }
  dispose() {
