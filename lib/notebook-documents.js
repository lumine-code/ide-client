...
class NotebookRecord {
  constructor(manager, { filePath, notebookType, metadata, show }) {
    ...
    // it, removed when that session stops or fails. A consumer standing its
    // own route down (linter-ruff's CLI) reads this, so it must go empty when
    // the diagnostics it defers to can no longer arrive.
  ...
  syncedCells(session) {
    ...
    return this.cells.filter((cell) => {
      ...
      if (session.adapter.transformDocumentText && !this.primaryEditor(cell)) return false;
  ...
  toTextDocumentItem(cell, session) {
    ...
    return {
      ...
      text: editor ? session.transformDocumentText(text, editor, uri) : text,
    };
  }
...
module.exports = class NotebookDocuments {
  ...
  // stand down — linter-ruff's CLI route asks this before double-reporting
  // what ide-ruff's server already covers. Empty when no bridge is open for
  // the path, which is also the answer when nothing bridges notebooks.
  ...
  applyCells(record, nextCells) {
    ...
    for (const cell of previous) {
      ...
      const key = C.uriKey(record.uriForCell(cell.id));
      const binding = this.manager.externalUris.get(key);
      if (binding?.record === record && !binding.editor) this.manager.externalUris.delete(key);
    }
    for (const cell of record.cells) {
      if (cell.kind !== "code") continue;
    ...
    for (const [session, state] of record.sessionState) {
      ...
      if (reopened.length) {
        ...
        diff.changed = true;
        diff.splice = { start, deleteCount: end - start, ids: nextIds.slice(start, end) };
      }
    ...
    if (sessionChanges.length) record.version++;
    for (const {
      session,
      state,
      nextIds,
      ...
      for (const id of [...diff.removed, ...reopened])
        session.releaseNotebookCell(record.uriForCell(id));
      for (const id of [...diff.added, ...reopened]) {
  ...
  sendCellChange(record, cellId, uri, contentChanges, editor) {
    for (const [session, state] of record.sessionState) {
      ...
      if (session.adapter.transformDocumentText && !editor) continue;
      const changes = session.adapter.transformDocumentText
        ? [{ text: session.transformDocumentText(editor.getText(), editor, uri) }]
        : contentChanges;
      state.version = record.version;
      session.changeNotebook(
        { uri: record.uri, version: record.version },
