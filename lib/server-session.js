...
module.exports = class ServerSession {
  ...
  supports(method, editor, feature = METHOD_FEATURES[method]) {
    ...
    if (method === "workspace/diagnostic")
      return !!this.capabilities.diagnosticProvider?.workspaceDiagnostics;
  ...
  installClientHandlers() {
    ...
    this.connection.onRequest("workspace/applyEdit", ({ edit, label }) =>
      this.manager.applyWorkspaceEditDetailed(edit, label, this),
    );
  ...
  async openEditor(editor) {
    ...
    const document = {
      ...
      uri,
      version: 1,
      wireOpen: sync.openClose,
      subscriptions: new CompositeDisposable(),
    };
    this.documents.set(key, document);
    document.openPromise = sync.openClose
    ...
    if (sync.save)
      document.subscriptions.add(
        editor.onDidSave(() => {
          ...
          if (sync.includeText)
            params.text = this.transformDocumentText(editor.getText(), editor, uri);
  ...
  // Result IDs belong to one diagnostic provider registration. A server may
  // withdraw it and register another identifier without restarting, so neither
  // document nor workspace IDs can cross that boundary. Stored diagnostics are
  // intentionally left alone: they may have arrived through publishDiagnostics
  // and cannot be distinguished from pull results after the fact.
  resetDiagnosticPullState() {
    ...
    for (const document of this.documents.values()) delete document.diagnosticResultId;
    this.refreshDiagnostics();
  }

  publishDiagnosticReport(uri, report, version) {
    if (!report || report.kind === "unchanged") return;
    return this.manager.publishDiagnostics(this, {
      uri,
      version,
  ...
  supportsWorkspaceDiagnostics() {
    const provider = this.capabilityOptions("textDocument/diagnostic");
    return (
      this.state === "running" &&
      !!(provider && typeof provider === "object" && provider.workspaceDiagnostics) &&
  ...
  processWorkspaceDiagnosticItems(items) {
    for (const report of items || []) {
      if (!report?.uri) continue;
      if (report.kind === "unchanged") {
        if (report.resultId)
          this.workspaceDiagnosticResultIds.set(C.uriKey(report.uri), {
            uri: report.uri,
            value: report.resultId,
          });
      } else {
        const accepted = this.publishDiagnosticReport(report.uri, report, report.version);
        if (accepted !== false) {
          if (report.resultId)
            this.workspaceDiagnosticResultIds.set(C.uriKey(report.uri), {
              uri: report.uri,
          ...
          else this.workspaceDiagnosticResultIds.delete(C.uriKey(report.uri));
        }
      }
    }
  }

  previousWorkspaceDiagnosticResultIds() {
    return [...this.workspaceDiagnosticResultIds.values()];
  }

  async pullWorkspaceDiagnostics() {
    if (!this.supportsWorkspaceDiagnostics()) return;
    if (this.workspaceDiagnosticPromise) {
      this.workspaceDiagnosticQueued = true;
      return this.workspaceDiagnosticPromise;
    }
    const token = `ide-client-workspace-diagnostic-${++this.partialResultCounter}`;
    const generation = this.diagnosticGeneration;
    const provider = this.capabilityOptions("textDocument/diagnostic");
    const params = {
      previousResultIds: this.previousWorkspaceDiagnosticResultIds(),
    ...
    this.partialResults.set(token, (partial) => {
      if (generation === this.diagnosticGeneration)
        this.processWorkspaceDiagnosticItems(partial?.items);
    });
    const pending = (async () => {
      try {
        const report = await this.request("workspace/diagnostic", params);
        if (this.state === "running" && generation === this.diagnosticGeneration)
          this.processWorkspaceDiagnosticItems(report?.items);
      } catch (error) {
    ...
    try {
      ...
      if (this.workspaceDiagnosticQueued) {
        this.workspaceDiagnosticQueued = false;
        this.scheduleWorkspaceDiagnostics(0);
      }
    }
  ...
  async pullDiagnostics(document) {
    ...
    const generation = this.diagnosticGeneration;
    const provider = this.capabilityOptions("textDocument/diagnostic", document.editor);
