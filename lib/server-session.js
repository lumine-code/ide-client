...
module.exports = class ServerSession {
  ...
  scheduleDiagnostics(document, delay = DIAGNOSTIC_DEBOUNCE_MS) {
    ...
    if (!this.supports("textDocument/diagnostic", document.editor)) {
  ...
  // session alive through the idle check, joining diagnostic pulls and
  // refreshes — but its content flows through the notebookDocument
  // notifications, so nothing here sends textDocument/didOpen for it. The
  // version is a getter into the notebook module's counter: the staleness
  // guard in publishDiagnostics compares against the version the module
  // stamped on the wire.
